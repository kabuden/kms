...
### 📄 일일 리포트 (자동 작성·보관)
사이클 ④(미국장 마감 후)마다 그날의 흐름을 마크다운 리포트로 자동 작성한다.
신뢰도, 시점별 예측 변화, 뉴스 요약, 종목별 공식 예측 vs 실제, 기간별 목표주가
하이라이트, 장기예측 정산, 발전 로그가 담긴다. 리포트는 DB와 `data/reports/`
폴더(`<날짜>.md`)에 함께 남고, 대시보드의 **일일 리포트** 섹션에서 날짜별로 보거나
내려받을 수 있다.
> 무료 호스팅은 디스크가 휘발성이라 재배포 시 초기화된다. 영구 보관은 render.yaml
> 의 유료 디스크(SA_DB_DIR)를 붙이면 된다.
...
│   ├── report.py               # 사이클 마감 일일 리포트(마크다운) 생성
